...
def edit_file(filename):
    file_path = filename
    try:
        with open('replica_2/'+file_path, 'r') as file:
    ...
    try:
        ...
        with open('replica_2/'+file_path, 'w') as file:
            file.writelines(lines)
...
def delete_file(filename):
    if os.path.exists(filename) or os.path.exists("files/"+filename ) or os.path.exists("replica_2/"+filename):
        try:
